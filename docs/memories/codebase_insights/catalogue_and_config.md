...
## catalogue.json
50 public MCP servers (as of recent research additions). Each entry has: `name`, `description`, `command` (for stdio), `url` (for SSE/HTTP), `tags`, `tech_stack`, `runtime`, `env_vars`, and optional `estimated_tools`, steering fields.
